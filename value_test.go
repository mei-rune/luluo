...
func TestGetComparer(t *testing.T) {
	...
	mi := MustToValue(int64(math.MaxInt64))
	if mi.IntValue() != math.MaxInt64 {
		t.Error("want", mi.IntValue(), "got", math.MaxInt64)
	}

	mi = MustToValue(int64(math.MinInt64))
	if mi.IntValue() != math.MinInt64 {
		t.Error("want", mi.IntValue(), "got", math.MinInt64)
	}

	mi = MustToValue(uint64(math.MaxUint64))
	if mi.UintValue() != math.MaxUint64 {
		t.Error("want", mi.UintValue(), "got", uint64(math.MaxUint64))
	}

	mi = MustToValue(uint64(0))
	if mi.UintValue() != 0 {
		t.Error("want", mi.UintValue(), "got", 0)
	}
