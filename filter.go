...
type Context interface {
	GetValue(name Name) (Value, error)
}

type GetValueFunc func(name Name) (Value, error)

func (f GetValueFunc) GetValue(name Name) (Value, error) {
	return f(name)
}
...
func Like(left, right func(Context) (Value, error)) func(Context) (bool, error) {
	return func(ctx Context) (bool, error) {
		...
		if leftValue.Type != ValueString {
			return false, errorWrap(err, "left operant isnot string")
		}
		leftStr := leftValue.StrValue()

		if rightValue.Type != ValueString {
			return false, errorWrap(err, "right operant isnot string")
		}
		rightStr := rightValue.StrValue()
